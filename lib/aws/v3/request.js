...
const { handleV3Error, buildV3ProviderError } = require('./error-utils');
  ...
  const request = await requestQueue.add(async () => {
    ...
    while (true) {
      ...
      try {
        ...
        // Use the shared classifier (error-utils) so retry decisions match how errors are
        // ultimately reported. It recognizes throttling by error name (Throttling /
        // ThrottlingException / TooManyRequestsException), which the previous inline check
        // missed: Query-protocol services like CloudFormation return "Rate exceeded" as
        // HTTP 400 with no $retryable flag, so name-based classification is required to retry.
