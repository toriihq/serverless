...
const impl = {
  ...
  addCredentials: (results, credentials) => {
    if (impl.validCredentials(credentials)) {
      Object.assign(results, credentials);
    }
  },
  /**
  ...
  addEnvironmentCredentials: (results, prefix) => {
    if (prefix) {
      ...
      if (accessKeyId && secretAccessKey) {
        impl.addCredentials(results, { accessKeyId, secretAccessKey, sessionToken });
      }
    }
  },
  /**
   ...
   */
  addProfileCredentials: async (results, profile) => {
    if (profile) {
      const fromIniOptions = { profile };
      if (process.env.AWS_SHARED_CREDENTIALS_FILE) {
        fromIniOptions.filepath = process.env.AWS_SHARED_CREDENTIALS_FILE;
      }
      // MFA token provider for interactive MFA prompts
      fromIniOptions.mfaCodeProvider = async (mfaSerial) => {
      ...
      try {
        const creds = await fromIni(fromIniOptions)();
        ...
        impl.addCredentials(results, { ...creds, profile });
      } catch (err) {
   ...
   */
  addEnvironmentProfile: async (results, prefix) => {
    if (prefix) {
      const profile = process.env[`${prefix}_PROFILE`];
      await impl.addProfileCredentials(results, profile);
    }
  },
};
...
class AwsProvider {
  ...
  async request(service, method, params, options) {
    ...
    const credentials = Object.assign({}, await this.getCredentials());
  ...
  async getCredentials() {
    ...
    try {
      await impl.addProfileCredentials(result, awsDefaultProfile);
    } catch (err) {
      if (err.code !== 'UNRECOGNIZED_AWS_PROFILE') throw err;
    }
    if (this.serverless.service.provider.profile && !this.options['aws-profile']) {
      // config profile
      await impl.addProfileCredentials(result, this.serverless.service.provider.profile);
    }
    impl.addEnvironmentCredentials(result, 'AWS'); // creds for all stages
    try {
      await impl.addEnvironmentProfile(result, 'AWS');
    } catch (err) {
      if (err.code !== 'UNRECOGNIZED_AWS_PROFILE') throw err;
    }
    impl.addEnvironmentCredentials(result, `AWS_${stageUpper}`); // stage specific creds
    try {
      await impl.addEnvironmentProfile(result, `AWS_${stageUpper}`);
    } catch (err) {
      if (err.code !== 'UNRECOGNIZED_AWS_PROFILE') throw err;
    }
    if (this.options['aws-profile']) {
      await impl.addProfileCredentials(result, this.options['aws-profile']); // CLI option profile
    }
